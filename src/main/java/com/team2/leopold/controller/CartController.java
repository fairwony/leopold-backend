...
public class CartController {
    ...
    public ResponseEntity<?> findCartList(@RequestParam(name = "uid") Integer userUid) {
        ...
        for (Cart c : cartList) {
            ResponseCartDto dto = new ResponseCartDto(c.getUid(), c.getQuantity(), c.getProduct().getCoverUrl(), c.getProduct().getPrice(), c.getProduct().getDiscountRate(), c.getProduct().getStock(), c.getProduct().getProductCategory().getName(), c.getProduct().getColor(), c.getProduct().getEngraving(), c.getProduct().getSwitchValue());
            dtoList.add(dto);
        }
    ...
    public ResponseEntity<?> changeQuantity(@RequestParam(name = "uid") Integer uid, @RequestParam(name = "quantity") Integer quantity) {
        try {
