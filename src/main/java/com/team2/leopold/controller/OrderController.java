...
public class OrderController {
    ...
    /* 주문 취소 */
    @DeleteMapping("/order")
    public ResponseEntity<?> deleteOrder(@RequestParam(name = "orderUid") Integer orderUid, HttpServletRequest request) {
        HttpSession session = request.getSession(false);
        if (session == null) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("로그인되어 있지 않음.");
    ...
    public ResponseEntity<?> findOrderList(HttpServletRequest request) {
        ...
        List<ResponseOrderPageDto> orderPageDtoList = orderService.findOrderListByUserUid((Integer) session.getAttribute("userUid"));
